...
const makeServices = async (heading,services) => {
  ...
  for (let i = 0; i < services.length; i++) {
    ...
    if (fs.existsSync(`${pwd}/${heading}/${serviceName}`)) {
      ...
      console.log(`DEBUG: 3 - Creating folder for service ${serviceName}: ${pwd}/${heading}/${serviceName}`);
      await cmdGet(`mkdir -p ${pwd}/${heading}/${serviceName}`);
      console.log(`DEBUG: 4 - Creating basic boilerplate for the service ${serviceName}`);
