...
const makeServices = async (heading, services) => {
  ...
  await cmdGet(`mkdir ${pwd}/${heading}`);
  ...
  for (let i = 0; i < services.length; i++) {
    ...
    await cmdGet(`mkdir ${pwd}/${heading}/${Object.keys(service)[0]}`);
    ...
    await runCopy(`${pwd}/apifieSample/*`, eachMicroservice);
    ...
    for (let j = 0; j < modelsList.length; j++) {
      ...
      console.log(`14 -cd ${eachMicroservice} && ${eachMicroservice}node_modules/.bin/sequelize model:generate --name ${model} --attributes ${stringAttributes} --models-path ${pwd}/services/service2/src/models && cd ${pwd}`);
      //await sleep(1000);
      checkIfRunningss(`cd ${eachMicroservice}`);
      checkIfRunningss(`${eachMicroservice}node_modules/.bin/sequelize model:generate --name ${model} --attributes ${stringAttributes} --models-path /Users/paridhimohindra/digitalWorkbench/nodester/services/service2/src/models`);
...
const checkIfRunningss = async (command) =>{
  await cmdGet(command);
  await sleep(1000);
};

const runCopy = async (src, dest) => {
  await cmdGet(`cp -r ${src} ${dest}`);
  await sleep(1000);
};

const installPackages = async (src, dest) => {
  await cmdGet(`cd ${dest} && npm install && cd ${src}`);
  //await sleep(1000);
};
